      ...
      base.class_eval do
        ...
        def send_mail
          if self.journalized.is_a?(Issue) && self.notes.present?
            ...
            users_regex=users.collect{|u| "\"#{Regexp.escape("#{u.firstname} #{u.lastname}#{" - #{u.mail}" unless u.pref.hide_mail}")}\":#{Setting.protocol}://#{Setting.host_name}#{Rails.application.routes.url_helpers.user_path(u)}".gsub(/(["\/])/, '\\\\\1')}.join('|')
